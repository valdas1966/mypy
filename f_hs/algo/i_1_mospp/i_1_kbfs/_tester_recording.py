...
============================================================================
 ...
 traces the exact same 14-push / 14-pop sequence as OMSPP's
 canonical recording pin — but the recorder shim rewrites
 the 3 `on_goal` events as `on_start` (with `start_index`
 instead of `goal_index`).

 Discovery order on this map: (3,0) at g=3 (start_idx=1) →
 (3,3) at g=6 (start_idx=2) → (0,3) at g=7 (start_idx=0;
 final pop terminates the search). All `reason='expanded'`.
============================================================================
"""
...
def test_recording_canonical_mospp_full_event_stream() -> None:
    ...
    ========================================================================
     ...
     Mirror of OMSPP `KBFS`'s canonical pin with `on_goal` →
     `on_start` and `goal_index` → `start_index`.
    ========================================================================
    """
