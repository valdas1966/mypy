...
| `_tester_recording.py` | Full event-stream pins (one per scenario; each is a single `actual == expected` assertion). The full pin implicitly verifies `_enrich_event` no-op (no h/f fields). Scenarios: canonical `grid_4x4_obstacle`, graph_abc, graph_diamond, graph_decrease (decrease_g, no h/f), grid_3x3 | 5 |
