"""
===============================================================================
 Script: sample N random ClusterDiamonds per GridMap as candidate START
 cells for OMSPP problems, stream the metadata to a single CSV, and upload
 the CSV to Google Drive.
-------------------------------------------------------------------------------
 OMSPP has one shared start s and many goals t1..tk. Step i_0 of the OMSPP
 experimental pipeline samples the START pool — single-cell clusters
 (steps=0, min_cells=1) so each row pins one candidate start cell. Goal
 clusters and problem assembly come later (i_1+, mirroring the MOSPP
 pipeline shape).
