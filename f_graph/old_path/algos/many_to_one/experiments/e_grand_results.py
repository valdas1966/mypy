...
def changed_nodes_by_depth() -> None:
    ...
    """
    df = pd.read_csv(csv_results_formatted)
    df['pct_changed_1'] = round(df['changed_1'] / df['old_nodes'] * 100, 2)
    df['pct_changed_2'] = round(df['changed_2'] / df['old_nodes'] * 100, 2)
    df['pct_changed_3'] = round(df['changed_3'] / df['old_nodes'] * 100, 2)
    df['pct_changed_4'] = round(df['changed_4'] / df['old_nodes'] * 100, 2)
    df['pct_changed_5'] = round(df['changed_5'] / df['old_nodes'] * 100, 2)
...
def explored_by_nodes_cnt() -> None:
    """
    ========================================================================
     Show the explored by old_nodes.
    ========================================================================
    """
    df = pd.read_csv(csv_results)
    ...
    df = df.sort_values(by='old_nodes')
    x = [str(int(val//1000)+100) for val in df['old_nodes'].tolist()]
    y = df['pct'].tolist()
