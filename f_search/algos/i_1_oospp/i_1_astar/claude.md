...
## Inheritance
- **Base Classes:** `AlgoOOSPP[ProblemOOSPP, SolutionOOSPP]`
- **Inherits From:**
  - `AlgoOOSPP`: OOSPP-specific algorithm base
...
- `_stats`: StatsOOSPP metrics tracker
...
3. **Post-processing:**
   ...
   - Returns `SolutionOOSPP` with validity flag, path, and stats
...
- Returns `SolutionOOSPP` instance
...
- Problem type: Single start, single goal (OOSPP)
