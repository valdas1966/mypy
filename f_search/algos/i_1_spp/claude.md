# i_1_spp - One-to-One Shortest Path Problem Algorithms

## Purpose
Contains all algorithm implementations for SPP (One-to-One Shortest Path Problem) - finding the shortest path from a single start state to a single goal state on a grid.

## Structure

- **i_0_base/** - `AlgoSPP` - Base class for all SPP algorithms
...
AlgoSearch[Problem, Solution]
  │
  └─ AlgoSPP[ProblemSPP, SolutionSPP] (i_0_base)
      │
      ├─ AStar (i_1_astar)
      │
...
Provides the foundation for all SPP algorithms:

### Type Constraints
- **Problem**: Bounded to `ProblemSPP` (has start and goal)
- **Solution**: Bounded to `SolutionSPP` (contains single path)
- **Stats**: Uses `StatsSPP` for metrics
...
### Specialization
- Refines stats type from `StatsSearch` to `StatsSPP`
...
All SPP algorithms follow this pattern:
...
To add a new SPP algorithm:

```python
class NewAlgo(AlgoSPP):
