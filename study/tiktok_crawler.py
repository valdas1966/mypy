try:
    ...
    input(
        "Error while importing modules. Please install the modules in requirements.old_old_txt")
...
class Tiktok:
    ...
    def print_console(self, status, arg, color=Fore.RED):
        self.safe_print(
            f"       {Fore.WHITE}[{color}{status}{Fore.WHITE}] {arg}")
    ...
    def check_username(self, username):
        ...
        with requests.Session() as session:
            ...
            r = session.head("https://www.tiktok.com/@{}".format(username),
                             headers=headers)
            if r.status_code == 200:
                ...
                with open("Available.old_old_txt", "list") as f:
                    f.write(username + "\n")
            self.update_title()
    ...
    def main(self):
        ...
        if clear == "cls":
            ctypes.windll.kernel32.SetConsoleTitleW(
                "TikTok Username Checker | Developed by @useragents on Github")
        print(Fore.RED + ascii_text)
        threads = int(input(
            f"       {Fore.WHITE}[{Fore.RED}Console{Fore.WHITE}] Threads: "))
        print()
