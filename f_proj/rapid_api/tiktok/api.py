...
class TiktokAPI:
    ...
    @staticmethod
    def comments_from_videos(id_video: str) -> list[dict]:
        """
        ============================================================================
        ...
        while has_more and rows_added:
            ...
            if output:
                try:
                    ...
                    return [DataCommentsFromVideos(id_video=id_video,
                                                   is_ok=True,
                                                   is_found=False).to_dict()]
                has_more = bool(d['hasMore'])
                cursor = d['cursor']
                for d_comment in d['comments']:
                    data = DataCommentsFromVideos()
                    data.id_comment = d_comment['id']
                    data.is_ok = True
                    data.is_found = True
                    data.text = d_comment['text']
                    data.replies = d_comment['reply_total']
                    data.diggs = d_comment['digg_count']
                    data.created = d_comment['create_time']  
                    data.id_video = d_comment['video_id']
                    data.id_user = d_comment['user']['id']
                    data.id_user_unique = d_comment['user']['unique_id']
                    data.nick = d_comment['user']['nickname']
                    data.region = d_comment['user']['region']
                    data.aweme = d_comment['user']['aweme_count']
                    data.favorited = d_comment['user']['total_favorited']
                    data.followers = d_comment['user']['follower_count']
                    data.following = d_comment['user']['following_count']
                    rows.append(data.to_dict())
                    len_rows = len(rows)
                    if len_rows % 100 == 0:
                        print(f'Cached {len_rows} rows so far')
